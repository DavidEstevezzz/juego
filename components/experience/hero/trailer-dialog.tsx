...
export function TrailerDialog({ onOpenChange }: TrailerDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger
        render={
          <button
            ...
            data-hero-cta
            className="font-system inline-flex min-h-12 items-center gap-3 border border-[color:var(--border-scarlet)] bg-transparent px-6 text-[0.72rem] uppercase tracking-[0.2em] text-foreground transition-colors duration-200 hover:bg-scarlet focus-visible:bg-scarlet focus-ring"
          />
        }
      >
      ...
      >
        <DialogTitle className="font-system border-b border-[color:var(--border-subtle)] px-4 py-3 text-[0.7rem] uppercase tracking-[0.2em] text-steel">
          {hero.trailerTitle}
        </DialogTitle>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-[color:var(--border-subtle)] px-4 py-3">
          <button
            ...
            onClick={toggleSound}
            aria-pressed={!muted}
            className="font-system inline-flex min-h-11 items-center gap-2 border border-[color:var(--border-brass)] px-4 text-[0.7rem] uppercase tracking-[0.18em] text-steel transition-colors hover:border-brass hover:text-foreground focus-ring"
          >
            {muted ? (
          ...
          <button
            ...
            className="font-system inline-flex min-h-11 items-center gap-2 border border-[color:var(--border-scarlet)] px-4 text-[0.7rem] uppercase tracking-[0.18em] text-foreground transition-colors hover:bg-scarlet focus-ring"
          >
            {hero.closeTrailer}
          </button>
        </div>
